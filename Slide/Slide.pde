...
void sliding() {
  ...
  accX = g*(sin(angleX) - mu*(cos(angleX))); 
  accY = g*(sin(angleY) - mu*(cos(angleY))); 
  ...
  for (int i = 0; i < n; ++i) {
    ...
    vx = mu*(Nodes[i].getVx() + accX*1.0/frameRate); /* !friction  */
    vy = mu*(Nodes[i].getVy() + accY*1.0/frameRate); /* !friction */
